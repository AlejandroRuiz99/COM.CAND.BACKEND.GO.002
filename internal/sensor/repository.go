...
type Repository interface {
	...
	GetLatestReadings(ctx context.Context, sensorID string, limit int) ([]*SensorReading, error)

	// GetReadingsByTimeRange obtiene lecturas en un rango temporal
	GetReadingsByTimeRange(ctx context.Context, sensorID string, start, end time.Time) ([]*SensorReading, error)

	// SaveConfig guarda o actualiza la configuración de un sensor
	SaveConfig(ctx context.Context, config *SensorConfig) error

	// GetConfig obtiene la configuración de un sensor
	GetConfig(ctx context.Context, sensorID string) (*SensorConfig, error)
